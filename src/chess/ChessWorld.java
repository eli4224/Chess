...
public class ChessWorld extends World<Piece> {
  ...
  private void runAI() {
    ...
    grid.getOccupiedLocations().stream().map(grid::get).parallel().map(x -> Chess.sha1(x.getLocation(), x.getClass(), x.getPlayerColor())).forEach(x -> {
      for(int i = 0; i < 20; i++){
        workingSeed[i]^=x[i];
      }
    });
    ...
    if(workingState == null) {
      ...
      State.stateMap.put(new ByteArrayWrapper(workingSeed), workingState);
    }
